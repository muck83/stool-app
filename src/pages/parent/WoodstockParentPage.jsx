...
function ConceptCard({ card, index, activeStage }) {
  ...
  const colors = [WS, '#185FA5', WG, '#A35E08', '#3F3A8F']
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem', marginBottom: '1.5rem' }}>
        {pillars.map((p, i) => (
          <div key={i} style={{
            ...
            background: 'var(--surface-2)',
            border: '1px solid var(--border)',
            borderTop: `3px solid ${i < 2 ? WS : i < 4 ? WG : '#A35E08'}`,
            borderRadius: '0 0 var(--r) var(--r)',
          }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {mod.universityDestinations.map((d, i) => (
          <div key={i} style={{
            ...
            border: '1px solid var(--border)',
            borderLeft: `4px solid ${[WS, '#185FA5', WG, '#A35E08'][i]}`,
            borderRadius: '0 var(--r) var(--r) 0',
            background: 'var(--surface)',
          }}>
  ...
  return (
    <div style={{
      ...
      {!revealed ? (
        ...
        <div style={{ padding: '1rem 1.2rem 1.25rem', borderTop: `1px solid ${col}22` }}>
          <div style={{ marginBottom: '1rem' }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#0E8A5F', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 6 }}>
              With this context
            </div>
            <div style={{
              fontSize: 13.5, color: 'var(--ink)', lineHeight: 1.75,
              padding: '.75rem 1rem', background: '#F0FDF4',
              borderRadius: 'var(--r)', borderLeft: '3px solid #0E8A5F',
            }}>
              {s.withUnderstanding}
            </div>
          </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg, #F7F7F5)' }}>
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '2rem 1.25rem 4rem' }}>
        ...
        {activeSection === 'research' && mod.researchInsights && (
          <div>
            <div style={{ marginBottom: '1.75rem' }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: WS, textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 6 }}>
                Evidence &amp; research
              ...
              <p style={{ fontSize: 13.5, color: 'var(--ink-3)', lineHeight: 1.65, margin: 0 }}>
                Seven evidence-based reframes drawn from the learning-science, mindset, parenting, and adolescent-development
                literature. Each insight names the folk belief, the evidence, and what the evidence suggests you actually do.
              </p>
            </div>
            {mod.researchInsights.map((r, i) => (
              <div key={r.id} style={{
                border: '1px solid var(--border)',
                borderRadius: 14,
                background: 'var(--card)',
                padding: '1.25rem 1.4rem',
                marginBottom: '1rem',
                boxShadow: '0 1px 2px rgba(17,16,13,.03)',
              }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: WS, textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 6 }}>
                  Insight {i + 1}
                </div>
                <h3 style={{ fontFamily: 'var(--serif, Georgia, serif)', fontSize: '1.1rem', color: 'var(--ink)', margin: '0 0 6px', lineHeight: 1.3 }}>
                  {r.title}
                </h3>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: WS, margin: '0 0 10px', fontStyle: 'italic' }}>
                  {r.headlineClaim}
                </div>
                <div style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.65, marginBottom: 10 }}>
                  <strong style={{ color: 'var(--ink)' }}>What the evidence shows:</strong> {r.evidence}
                </div>
                <div style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.65, marginBottom: 10 }}>
                  <strong style={{ color: 'var(--ink)' }}>So what:</strong> {r.soWhat}
                </div>
                <div style={{
                  fontSize: 13, color: 'var(--ink)', lineHeight: 1.6,
                  padding: '10px 12px',
                  background: 'var(--teal-light, #DCF0E6)',
                  borderLeft: `3px solid ${WS}`,
                  borderRadius: 6,
                  marginBottom: 10,
                }}>
                  <strong>Try this:</strong> {r.action}
                </div>
                {r.citations && r.citations.length > 0 && (
                  <details style={{ fontSize: 12, color: 'var(--ink-3)' }}>
                    ...
                    <ul style={{ margin: '6px 0 0 1.1rem', padding: 0, lineHeight: 1.5 }}>
                      {r.citations.map((c, j) => <li key={j} style={{ marginBottom: 4 }}>{c}</li>)}
                    </ul>
                  </details>
                )}
              </div>
