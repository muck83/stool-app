  ...
  return (
    <div style={{ display: 'flex', gap: 4 }}>
      ...
      {record.status !== 'removed' && (
        <button onClick={() => onSetStatus(record.id, 'removed')} disabled={updating === record.id} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 6, border: '1px solid #BF4820', color: '#BF4820', background: 'white', cursor: 'pointer' }}>
          Remove
        </button>
      )}
      {record.status === 'removed' && (
    ...
    <div>
      ...
      <StatGrid
        ...
        items={[
          ...
          ['Flagged', counts.flagged, '#92400E'],
          ['Verified', counts.verified, '#185FA5'],
          ...(statsColumns > 4 ? [['Removed', counts.removed, '#BF4820']] : []),
        ]}
      />
      ...
      <div style={{ background: 'white', borderRadius: 8, border: '1px solid var(--border)', overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#F7F6F3' }}>
                {['Date', 'Email', 'Teacher', 'School', 'Current post', 'Considering', 'Diagnostic', 'School score', 'Status', 'Actions', 'Detail'].map((label) => (
                  <th key={label} style={{ padding: '8px 12px', textAlign: 'left', fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--ink-4)', borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap' }}>
                    {label}
                  </th>
                ))}
              ...
              {filtered.map((record) => {
                ...
                return (
                  <Fragment key={record.id}>
                    <tr style={{ borderBottom: '1px solid var(--border)', opacity: record.status === 'removed' ? 0.5 : 1 }}>
                      <td style={{ padding: '7px 12px', color: 'var(--ink-4)' }}>{formatDate(record.created_at)}</td>
                      <td style={{ padding: '7px 12px', fontSize: 11, color: record.email ? 'var(--ink-2)' : 'var(--ink-4)' }}>{record.email || '—'}</td>
                      ...
                      <tr style={{ borderBottom: '1px solid var(--border)', background: '#FAFAF9' }}>
                        <td colSpan={11} style={{ padding: '1rem 1.25rem' }}>
                          <div style={{ display: 'grid', gridTemplateColumns: '1.35fr 1fr', gap: '1rem' }}>
                            <div>
                              <div style={{ fontSize: 11, fontWeight: 600, color: '#A35E08', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: '.5rem' }}>
                                Teacher snapshot
                              </div>
...
function EmailsPanel({ adminSupabase }) {
  ...
  const load = async () => {
    if (!adminSupabase) return
    setLoading(true)
    // Pull all emails from diagnostic_submissions AND profiles table, deduplicate
    const [diagRes, profRes] = await Promise.all([
      adminSupabase.from('diagnostic_submissions').select('email, name, current_country, current_city, school, diagnosis_kind, created_at').not('email', 'is', null).order('created_at', { ascending: false }),
      adminSupabase.from('profiles').select('email, profile, updated_at').order('updated_at', { ascending: false }),
    ])

    const map = new Map()
    // Profile emails first (richer data)
    ;(profRes.data || []).forEach(r => {
      if (!r.email) return
      map.set(r.email, {
        email: r.email,
        name: r.profile?.name || null,
        country: r.profile?.cc || null,
        city: r.profile?.city || null,
        school: r.profile?.school || null,
        source: 'profile',
        date: r.updated_at,
      })
    })
    // Diagnostic emails (may add entries not in profiles)
    ;(diagRes.data || []).forEach(r => {
      if (!r.email) return
      if (!map.has(r.email)) {
        map.set(r.email, {
          email: r.email,
          name: r.name || null,
          country: r.current_country || null,
          city: r.current_city || null,
    ...
    setRows([...map.values()].sort((a, b) => new Date(b.date) - new Date(a.date)))
    setLoading(false)
  }
  ...
  const filtered = rows.filter(r => {
    if (!search) return true
    return [r.email, r.name, r.country, r.city, r.school].join(' ').toLowerCase().includes(search.toLowerCase())
  })

  const csvExport = () => {
    const header = 'Email,Name,Country,City,School,Source,Date'
    const body = filtered.map(r => [r.email, r.name || '', r.country || '', r.city || '', r.school || '', r.source, formatDate(r.date)].map(v => `"${v}"`).join(',')).join('\n')
    const blob = new Blob([header + '\n' + body], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = 'stool_emails.csv'; a.click()
  ...
  return (
    <div>
      <StatGrid columns={3} items={[
        ['Total emails', rows.length, '#1a1917'],
        ['From profiles', rows.filter(r => r.source === 'profile').length, '#185FA5'],
        ['From diagnostic', rows.filter(r => r.source === 'diagnostic').length, '#3F3A8F'],
      ]} />
      <div style={{ display: 'flex', gap: 8, marginBottom: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
        <input
          value={search}
          onChange={e => setSearch(e.target.value)}
          placeholder="Search email, name, country, school..."
          style={{ padding: '5px 12px', border: '1px solid var(--border-2)', borderRadius: 20, fontSize: 12, flex: 1, minWidth: 240 }}
        />
        <button onClick={load} style={{ fontSize: 12, padding: '5px 14px', border: '1px solid var(--border-2)', borderRadius: 20, background: 'white', cursor: 'pointer' }}>
          {loading ? 'Loading...' : 'Refresh'}
        </button>
        <button onClick={csvExport} style={{ fontSize: 12, padding: '5px 14px', border: '1px solid var(--border-2)', borderRadius: 20, background: 'white', cursor: 'pointer' }}>
          Export CSV
        </button>
      </div>
      <div style={{ background: 'white', borderRadius: 8, border: '1px solid var(--border)', overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#F7F6F3' }}>
                {['Email', 'Name', 'Country', 'City', 'School', 'Source', 'Date'].map(label => (
                  <th key={label} style={{ padding: '8px 12px', textAlign: 'left', fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--ink-4)', borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap' }}>
                    {label}
                  </th>
                ))}
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} style={{ padding: '2rem', textAlign: 'center', color: 'var(--ink-4)', fontSize: 13 }}>{loading ? 'Loading…' : 'No emails collected yet.'}</td></tr>
              ) : filtered.map((r, i) => (
                <tr key={r.email} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'white' : '#FAFAF9' }}>
                  <td style={{ padding: '7px 12px', fontWeight: 500 }}>{r.email}</td>
                  <td style={{ padding: '7px 12px', color: 'var(--ink-3)' }}>{r.name || '—'}</td>
                  <td style={{ padding: '7px 12px', color: 'var(--ink-3)' }}>{r.country || '—'}</td>
                  <td style={{ padding: '7px 12px', color: 'var(--ink-3)' }}>{r.city || '—'}</td>
                  <td style={{ padding: '7px 12px', color: 'var(--ink-3)' }}>{r.school || '—'}</td>
                  <td style={{ padding: '7px 12px' }}>
                    <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 10, background: r.source === 'profile' ? '#E6F1FB' : '#E5E3F5', color: r.source === 'profile' ? '#185FA5' : '#3F3A8F', textTransform: 'capitalize' }}>
                      {r.source}
                    </span>
                  </td>
                  <td style={{ padding: '7px 12px', color: 'var(--ink-4)' }}>{formatDate(r.date)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function Admin() {
  ...
  if (!ADMIN_PASSWORD || !adminSupabase) {
    return (
      <div style={{ padding: '2rem', color: '#BF4820' }}>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F7F6F3' }}>
        <div style={{ background: 'white', borderRadius: 12, padding: '2rem', width: 320, boxShadow: '0 4px 24px rgba(0,0,0,.08)' }}>
          ...
          <input
            ...
            autoFocus
            style={{ width: '100%', padding: '10px 12px', border: `1px solid ${pwError ? '#BF4820' : 'var(--border-2)'}`, borderRadius: 'var(--r)', fontSize: 14, marginBottom: 8, boxSizing: 'border-box' }}
          />
          {pwError && <div style={{ fontSize: 12, color: '#BF4820', marginBottom: 8 }}>Wrong password</div>}
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: '1.5rem', background: 'white', border: '1px solid var(--border)', borderRadius: 8, padding: 4, width: 'fit-content', flexWrap: 'wrap' }}>
          {[
            ...
            { id: 'emails', label: 'Emails collected' },
          ].map((item) => (
            <button
              key={item.id}
        ...
        {tab === 'emails' && <EmailsPanel adminSupabase={adminSupabase} />}
        {tab === 'salary' && (
          <BasicTablePanel
