...
export default function MyMove() {
  ...
  const DeltaChip = ({ pred, cur, label, col }) => {
    const diff = pred - cur
    const bg = diff > 0 ? '#E1F5EE' : diff < 0 ? '#FAECE7' : '#F5F5F4'
  ...
  return (
    <div className="tp active">
      ...
      {profile.dc && !selectedCountry && (
        <div style={{ background: '#EEEDFE', borderRadius: 'var(--r)', padding: '.625rem 1rem', marginBottom: '1rem', fontSize: 12, color: '#3C3489', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '.5rem' }}>
      ...
      {!searchLower && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '.65rem', marginBottom: '1.25rem' }}>
          {top3.map((r, i) => {
            ...
            const borderCol = diff > 0.5 ? 'var(--teal)' : diff > -0.5 ? '#534AB7' : 'var(--amber, #BA7517)'
            return (
              ...
              >
                ...
                <div style={{ display: 'grid', gap: 5 }}>
                  {[{ l: 'School', v: r.schPred, c: '#BA7517' }, { l: 'Place', v: r.plcPred, c: '#534AB7' }, { l: 'Package', v: r.pkgPred, c: '#1D9E75' }].map(b => (
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 10, background: 'var(--bg, white)', paddingTop: '.25rem', paddingBottom: '.5rem' }}>
        <div style={{ background: 'var(--surface-2)', borderRadius: 'var(--r)', padding: '.75rem 1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            {[
              { key: 'school', label: 'School', col: '#BA7517' },
              { key: 'place', label: 'Place', col: '#534AB7' },
              { key: 'package', label: 'Package', col: '#1D9E75' },
            ].map(w => (
      ...
      <div>
        {displayList.map((result) => {
          ...
          const diff = avgPred - curComposite
          const borderCol = diff > 0.5 ? 'var(--teal)' : diff > -0.5 ? 'var(--border)' : 'var(--coral, #D85A30)'
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '.65rem', marginBottom: '.35rem' }}>
                <div style={{ display: 'flex', gap: 3, flex: 1, alignItems: 'center' }}>
                  {[{ v: result.schPred, c: '#BA7517' }, { v: result.plcPred, c: '#534AB7' }, { v: result.pkgPred, c: '#1D9E75' }].map((b, i) => (
                    <MiniBar key={i} value={b.v} col={b.c} />
                  ))}
                </div>
                <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                  <DeltaChip pred={result.schPred} cur={curSch} label="S" col="#BA7517" />
                  <DeltaChip pred={result.plcPred} cur={curPlc} label="P" col="#534AB7" />
                  <DeltaChip pred={result.pkgPred} cur={curPkg} label="$" col="#1D9E75" />
                </div>
              </div>
