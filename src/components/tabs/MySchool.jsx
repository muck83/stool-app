...
import PDContextBanner from '../pd/PDContextBanner.jsx'
  ...
  return (
    <div className="sr-prog">
      {Array.from({ length: total }, (_, i) => (
        <div key={i} style={{ height: 4, flex: 1, borderRadius: 2, transition: 'background .3s', background: i < step ? '#A35E08' : i === step ? '#EF9F27' : 'rgba(26,25,23,.12)' }} />
      ))}
    </div>
  )
}
...
function ProfileCard({ school, country, answers, hours, noticePeriod }) {
  ...
  const oc = overall >= 8 ? '#0E8A5F' : overall >= 6 ? '#A35E08' : overall >= 4 ? '#BF4820' : '#A32D2D'
  ...
  return (
    <div>
      <div style={{ background: '#F5E5C6', padding: '1rem 1.25rem .875rem', borderRadius: 'var(--r) var(--r) 0 0', border: '1px solid rgba(163,94,8,.2)' }}>
...
const scoreColor = (s) => s >= 8 ? '#0E8A5F' : s >= 6 ? '#A35E08' : s >= 4 ? '#BF4820' : '#A32D2D'
                            ...
                            {['all', 'similar'].map(f => (
                              <button
                                ...
                                style={{
                                  ...
                                  border: b5Filter === f ? '1.5px solid #A35E08' : '1.5px solid var(--border-2)',
                                  background: b5Filter === f ? '#F5E5C6' : 'white',
                                  color: b5Filter === f ? '#A35E08' : 'var(--ink-3)',
...
export default function MySchool() {
  ...
  const col = q ? (SR_DIM_COLORS[q.key] || '#A35E08') : '#A35E08'
      ...
      <div className="tp active">
        ...
        {hasSchool && (
          <div
            ...
            style={{
              background: 'linear-gradient(135deg, #F5E5C6 0%, #FFF8EE 100%)',
              border: '1px solid rgba(163,94,8,.25)',
              borderRadius: 'var(--rl)',
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '.5rem' }}>
              {reviews.map((r, i) => {
                ...
                const oc = avg >= 8 ? '#0E8A5F' : avg >= 6 ? '#A35E08' : avg >= 4 ? '#BF4820' : '#A32D2D'
