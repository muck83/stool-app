...
export function Diagnostic() {
  ...
  const deltaColor = delta > 1 ? '#0E8A5F' : delta < -1 ? '#BF4820' : '#3F3A8F'
  ...
  return (
    <div className="tp active">
      <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 'var(--rl)', padding: '1.15rem 1.2rem', marginBottom: '1rem', boxShadow: '0 2px 18px rgba(0,0,0,.04)' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '.75rem', flexWrap: 'wrap', marginBottom: '.8rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem' }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--teal-dark)', background: 'var(--teal-light)', border: '1px solid rgba(14,138,95,.22)', borderRadius: 999, padding: '5px 10px', whiteSpace: 'nowrap' }}>
      ...
      <button className="btn btn-primary" style={{ marginTop: '1rem', maxWidth: 240, boxShadow: '0 8px 18px rgba(14,138,95,.16)' }} onClick={run}>
        Which leg is it? →
      </button>
      ...
      {result && (
        <div style={{ marginTop: '1.5rem', background: 'white', border: '1px solid var(--border)', borderRadius: 'var(--rl)', padding: '1.5rem', boxShadow: '0 2px 20px rgba(0,0,0,.04)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '.75rem', flexWrap: 'wrap', marginBottom: '.5rem' }}>
            ...
            {saveState === 'saved' && <div style={{ fontSize: 11.5, color: '#0E8A5F' }}>Saved privately for admin review.</div>}
            {saveState === 'offline' && <div style={{ fontSize: 11.5, color: 'var(--ink-4)' }}>Supabase is not configured, so this result stays local only.</div>}
            {saveState === 'error' && <div style={{ fontSize: 11.5, color: '#BF4820' }}>This result could not be saved to the admin dashboard.</div>}
          </div>

          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '.45rem', fontSize: 11, fontWeight: 600, color: 'var(--amber-dark)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: '.55rem', background: 'var(--amber-bg)', border: '1px solid rgba(163,94,8,.18)', borderRadius: 999, padding: '5px 10px' }}>
            {result.title}
          </div>
              ...
              {result.links.map(({ label, tab }) => (
                <button
                  ...
                  style={{
                    ...
                    color: 'var(--teal-dark)', background: '#DCF0E6',
                    border: '1.5px solid rgba(14,138,95,.3)', borderRadius: 'var(--r)',
            ...
            <div style={{ marginTop: '1.25rem', borderTop: '1px solid var(--border)', paddingTop: '1.25rem' }}>
              ...
              {!scoreApplied ? (
                <button
                  ...
                  style={{ fontSize: 12.5, fontWeight: 500, color: 'white', background: 'var(--amber)', border: 'none', borderRadius: 'var(--r)', padding: '7px 16px', cursor: 'pointer', boxShadow: '0 8px 18px rgba(163,94,8,.16)' }}
                >
                  Update my School score to {diagScore} -&gt;
                </button>
              ) : (
                <div style={{ fontSize: 12.5, color: 'var(--teal-dark)', background: '#DCF0E6', borderRadius: 'var(--r)', padding: '7px 14px', display: 'inline-block', fontWeight: 500 }}>
