...
function TraitBar({ trait, score }) {
  ...
  const isLow  = score <= 4
  const color  = isHigh ? 'var(--teal)' : isLow ? '#A35E08' : 'var(--ink-3)'
      ...
      {(profile.sch != null || profile.plc != null || profile.pkg != null) && (
        <div style={{ maxWidth: 720, marginBottom: '2.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '.75rem', flexWrap: 'wrap' }}>
            <LegPill label="School"  score={profile.sch} color="#A35E08" bg="#F5E5C6" textColor="#4A2A02" />
            <LegPill label="Place"   score={profile.plc} color="#3F3A8F" bg="#E5E3F5" textColor="#1A164E" />
            <LegPill label="Package" score={profile.pkg} color="#0E8A5F" bg="#DCF0E6" textColor="#06523A" />
          </div>
